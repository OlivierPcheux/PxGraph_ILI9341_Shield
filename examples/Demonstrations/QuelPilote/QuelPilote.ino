...
// Carte uno
  ...
  #define _UNO_
    ...
    #define _VRILLETTE_
      ...
      #define _MEGA_
    #else
      Il y a un problème, Px_graph ne fonctionne qu´avec une carte Uno ou Méga  
    #endif
  #endif
#endif
...
boolean _9341_=false; // On n'a pas une carte avec un ILI9341
boolean _9486_=false; // On n'a pas une carte avec un ILI9486
...
void setGraphModeST7781(void) // Initialisation de l'afficheur et de ses registres ainsi que certaines variables globales
{
...
void setup()
{
  ...
  setGraphModeILI9341(); // On a au moins besoin du reset, pour les registres, on peut y mettre n'importe quoi!
  ...
  
  {
    ...
    if ((pilote1==0x83)&&(pilote2==0x77))
    {
      ...
      setGraphModeST7781(); // Il faut initialiser l'afficheur
      text("\n\n Vous avez un\n     ST7781");
    }
    else
    
    { // Test si c'est un ILI9341
     ...
     if ((pilote1==0x93)&&(pilote2==0x41)) 
      { 
        ...
        setGraphModeILI9341(); // Il faut initialiser l'afficheur
        text("\n\n Vous avez un\n     ILI9341");
      }
     else if ((pilote1==0x94)&&(pilote2==0x86)) 
      { 
        Serial.println("  Vous avez un ILI9486");
        _9486_=true; // A faire avant le clrscr contenu dans setGraphMode9341() 
        setGraphModeILI9486(); // Il faut initialiser l'afficheur
        //text("\n\n Vous avez un\n     ILI9486");
        
      }
      else
    
      { // Test si c'est un RM68090
        writeRegister(0x00); // envoi de la commande 00 (ID code?)
        writeRegister(0x00); // envoi de la commande 00 (ID code?)
        pilote1=dataRead8Bits(); // récupération des poids forts
        pilote2=dataRead8Bits(); // récupération des poids faibles
        if ((pilote1==0x68)&&(pilote2==0x09))
        { 
          Serial.println("  Vous avez un RM68090");
          _9341_=true; // A faire avant le clrscr contenu dans setGraphMode9341()
          setGraphModeST7781(); // Il faut initialiser l'afficheur
          text("\n\n Vous avez un\n     RM68090");
        }
