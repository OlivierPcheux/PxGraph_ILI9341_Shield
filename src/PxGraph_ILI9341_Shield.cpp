...
#define _ILI9341_Shield_ // Pour un shield pour Uno/Mega avec un ST7781 (340x200) 
...
#elif defined (_ILI9486_8bits_)
  #include <PxGraph_ILI9486_Shield.h>
#endif
...
#define READ            READ_PORT         &= ~READ_MASK         // Lecture, met la broche RD à 0, doit durer au moins 355ns (ILI9341, ILI9486), 150ns (ST7781)
#define NO_READ         READ_PORT         |=  READ_MASK         // Fin de lecture, remet RD à 1, doit durer au moins 90ns (ILI9341, ILI9486), 150ns (ST7781)
...
#if defined (_ILI9341_Shield_) || defined(_ILI9486_8bits_)
...
void setGraphMode(byte mode) // Initialisation de l'afficheur et de ses registres ainsi que certaines variables globales
{
  ...
  #elif defined (_ILI9486_8bits_)
    // Paramétrage des registres du ILI9486
    // Registre 36 : MADCTL (Memory Access Control) lien entre la mémoire et l'afficheur
    writeRegister1B(0x36,0x08+(_graphMode_<<6)); //   4? : inversion petit côté,  8? : inversion grand côté
    writeRegister1B(0x3A,0x55); // Interface 16 bits par couleur
    writeRegister1B(0xB4,0x00); // Display Inversion Control
    writeRegister1B(0xC2,0x44); // Power Control 3
    writeRegister2B(0xC5,0x31,0x3C); // VCOM Control 1
    writeRegister(0x11); // Sleep OUT
    delay(10);
    writeRegister(0x29); // Display ON
...
{
  boolean contexte = drawInverse; // Sauvegarde de l'état de drawInverse
  drawInverse=INVERSE; // Pour dessiner le curseur
  vLine(x, 0, _max_y_); hLine(0, _max_x_, y); // Dessine une croix curseur
  delay(100);
  vLine(x, 0, _max_y_); hLine(0, _max_x_, y); // Efface la croix curseur
  drawInverse=contexte; // Restitution du contexte correct
}


//################################# CLAVIER #################################
// Ecrit comme un objet dans la première version, il est défini maintenant comme fonction se supperposant aux objets existants.
// Le clavier permet d'entrer un nom pour lire ou sauvegarder un fichier. Il n'est pas destiné à être utilisé pour écrire un livre.
String valeurClavier; // Chaîne de retour
const PROGMEM char azerty[] = {'1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'A', 'Z', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P', 'Q', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'W', 'X', 'C', 'V', 'B', 'N', ' ', '.', '/', ' '};


//############################## clavier40Draw ##############################
// Cete fonction permet de dessiner le clavier. Comme on est en mode INVERSE il faut le dessiner au moins deux fois
void clavier40Draw(int x1, int y1, String message)
{
  setTextSize(2); setTextSerif(NORMAL); // Ce qui se voit le mieux
  for (int ligne = 0; ligne < 4; ligne++)
  {
    if (ligne == 3) hLine(x1 + 30, x1 + 230, y1 + 100); // Dessin des touches, dernière ligne horizontale
    else hLine(x1 + ligne * 10, x1 + ligne * 10 + 210, y1 + ligne * 20 + 40); // Autres lignes horizontales
    vLine(x1 + ligne * 10 + 200, y1 + ligne * 20 + 21, y1 + ligne * 20 + 39); // petit trat vertical

    for (int colonne = 0; colonne < 10; colonne++)
    {
      // Dessine une touche
      vLine(x1 + colonne * 20 + ligne * 10,  y1 + ligne * 20 + 21, y1 + ligne * 20 + 39); // ligne verticale après la touche
      setTextCursor(x1 + colonne * 20 + ligne * 10 + 10 - getTextWidth(String(char(pgm_read_byte_near(azerty + colonne + 10 * ligne)))) / 2, y1 + ligne * 20 + 24); // Permet de centrer la lettre
      text(String(char(pgm_read_byte_near(azerty + colonne + 10 * ligne)))); // Lettre correpondnate
    }
  }
  // Dessine backSpace
  hLine(x1 + 213, x1 + 225, y1 + 89); line(x1 + 214, y1 + 88, x1 + 214 + 4, y1 + 88 - 4); line(x1 + 214, y1 + 90, x1 + 214 + 4, y1 + 90 + 4);
  // Dessine le résultat
  rect(x1, y1, x1 + 230, y1 + 20); // Dessin du cadre résultat
  setTextCursor(x1 + 3, y1 + 3);
  byte depart = 0; while (getTextWidth(valeurClavier.substring(depart)) > 224) depart++; // Si le texte est trop long, on n'affiche que la fin
  if (valeurClavier == "") text(message);
  else text(valeurClavier.substring(depart));
  // Retour à la normale
}


//################################ clavier40 ################################
String clavier40(int x1, int y1, String message)
{
  int x, y; // Coordonnés du point de touche
  valeurClavier = ""; // Sinon on garde la chaîne d'une fois sur l'autre
  drawInverse=INVERSE; // Inversion pour pouvoir l'effacer
  setTextOrient(NORMAL); // Cause: on mélange textes et dessins
  clavier40Draw(x1, y1, message);
  do
  {
    while (!isTouch()); // Attente d'un appui
    y = (getTouchY() - y1) / 20; x = (getTouchX() - x1 + 10 - y * 10) / 20; // Lecture des coordonnés du point de touche
    if ((x >= 0) && (x < 10) && (y >= 0) && (y < 5))
    {
      croixCurseur(x1 + x * 20 + y * 10, y1 + y * 20 + 10);
      if (y > 0) // On ajoute une lettre
      {
        clavier40Draw(x1, y1, message); // Efface le clavier
        x += y * 10 - 10; // Numéro de la touche
        if (x == 39) // Backspace
          valeurClavier = valeurClavier.substring(0, valeurClavier.length() - 1);
        else valeurClavier += String(char(pgm_read_byte_near(azerty + x)));
        clavier40Draw(x1, y1, message); // Dessine le clavier
      }
      else
      {
        clavier40Draw(x1, y1, message); // Efface le clavier
  ...
  } while (1); // Boucle infinie, on sort au milieu si on pointe sur la grande barre
}
